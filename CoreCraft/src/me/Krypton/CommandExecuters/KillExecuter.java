			...
			if(lenght == 1){
				...
				for (Player target : Bukkit.getServer().getOnlinePlayers()){
					if(target.getName().equalsIgnoreCase(args[0])){
						...
						target.sendMessage(ChatColor.BLUE + target.getDisplayName() + ": " + ChatColor.RED + "You were killed by: " + ChatColor.DARK_AQUA + player.getDisplayName());
						player.sendMessage(ChatColor.GREEN + "You have successfuly killed: " + ChatColor.DARK_AQUA + target.getDisplayName());
						playerFound = true;
						break;
					}
				}
