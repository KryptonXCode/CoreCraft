			...
			if(length == 1){
				...
				for (Player target : Bukkit.getServer().getOnlinePlayers()){
					if(target.getName().equalsIgnoreCase(args[0])){
						...
						target.sendMessage(ChatColor.BLUE + target.getDisplayName() + ": " + ChatColor.DARK_AQUA + "You were fed by: " + ChatColor.RED + player.getDisplayName());
						player.sendMessage(ChatColor.GREEN + "You have fed: " + ChatColor.DARK_AQUA + target.getDisplayName());
						playerFound = true;
						break;
					}
					return true;
